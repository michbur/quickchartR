...
test:
	...
	R --slave -e 'Sys.setenv(NOT_CRAN="true"); library(testthat); setwd(file.path(.libPaths()[1], "quickchartR", "tests")); system.time(test_check("quickchartR", filter="${file}", reporter=ifelse(nchar("${r}"), "${r}", "summary")))'
...
check: build
	-R CMD check --as-cran quickchartR_$(VERSION).tar.gz
	rm -rf quickchartR.Rcheck/
